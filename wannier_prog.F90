...
subroutine write_ukk()

    use w90_parameters, only: num_wann,num_kpts,num_bands,ndimwin,u_matrix_opt,&
                              u_matrix,lwindow,wannier_centres,exclude,alat
    use w90_io, only: stdout,seedname
    use w90_constants, only: dp, cmplx_0
    implicit none
    integer :: ik,iw,ibnd
    COMPLEX(KIND = DP), ALLOCATABLE :: u_kc(:, :, :)

    ALLOCATE(u_kc(num_bands-exclude, num_wann, num_kpts)) ! nbndep
    u_kc(:, :, :) = cmplx_0
    DO ik = 1, num_kpts
       !
       u_kc(1:ndimwin(ik), 1:num_wann, ik) = &
             MATMUL(u_matrix_opt(1:ndimwin(ik), :, ik), u_matrix( :,1:num_wann, ik))
       !
    ENDDO

    OPEN(1111, FILE =trim(seedname)//'.ukk', FORM = 'formatted')
    WRITE(1111, *) exclude+1,num_bands !ibndstart, ibndend
    DO ik = 1, num_kpts
       DO ibnd = 1, num_bands-exclude !num_bands
          DO iw = 1, num_wann
             WRITE(1111, *) u_kc(ibnd, iw, ik)
          ENDDO
       ENDDO
    ENDDO
    ...
    DO ik = 1, num_kpts
       DO ibnd = exclude+1,num_bands 
          WRITE(1111, *) lwindow(ibnd, ik)
       ENDDO
    ENDDO

    DO ibnd = 1, num_bands
       if (ibnd .le. exclude) then
           write(1111,*) "T"
       else
           WRITE(1111, *) "F" !!excluded_band(ibnd)
       endif
    ENDDO
    ...
    DO iw = 1, num_wann
       ...
       WRITE (1111, '(3E22.12)') wannier_centres(:,iw) /0.52917720859/alat !wann_centers(:, iw) / alat / bohr
    ENDDO
    close(1111)

end subroutine write_ukk
