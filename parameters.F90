...
! $![ysl-e]
  ...
  ! FC added 
  ! the number of bands excluded in writing the ukk file
  ! assuming 1-exclude bands are excluded
  integer, public, save :: exclude
  real(kind=dp),public,save :: alat
...
contains
  ...
  subroutine param_read()
    ...
    !%%%%%%%%%%%%%%%%
    exclude = 0
    call param_get_keyword('exclude', found, i_value=exclude)
    if (exclude < 0) call io_error('Error: exclude must be greater than zero')

    call param_get_keyword('alat', found, r_value=alat)
    if (.not. found) call io_error('Error: You must specify alat in bohr')
...
!===========================================================!
  subroutine param_dist
    ...
    integer :: ierr
    ! FC added 
    call comms_bcast(alat,1)
    call comms_bcast(exclude,1)
    ! FC
